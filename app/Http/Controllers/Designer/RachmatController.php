...
class RachmatController extends Controller
{
    ...
    public function store(StoreRachmaRequest $request)
    {
        try {
            ...
            $rachma = Rachma::create([
                ...
                'color_numbers' => [$request->color_numbers], // Store as array
                'price' => $request->price,
                'original_price' => $request->price, // Set original price same as price initially
                'files' => $filesData, // Store multiple files as array
    ...
    public function show($id)
    {
        ...
        $stats = [
            ...
            'average_rating' => $rachma->orders()->where('status', 'completed')->avg('rating') ?? 0,
        ];
    ...
    public function edit($id)
    {
        ...
        // Debug information for troubleshooting
        \Log::info('Edit Authorization Debug', [
            'user_id' => $user->id,
            'designer_id' => $designer->id,
            'rachma_id' => $rachma->id,
            'rachma_designer_id' => $rachma->designer_id,
        ...
        $rachma->load(['categories', 'parts']);
        $categories = Category::all();
        $partsSuggestions = \App\Models\PartsSuggestion::active()
            ->orderBy('name_ar')
            ->get(['id', 'name_ar', 'name_fr']);

        return Inertia::render('Designer/Rachmat/Edit', [
            'rachma' => $rachma,
            'categories' => $categories,
            'partsSuggestions' => $partsSuggestions,
        ]);
     ...
     */
    public function update(UpdateRachmaRequest $request, $id)
    {
        // Find the rachma by ID
        $rachma = Rachma::findOrFail($id);
        ...
        // Use strict comparison with type casting to ensure proper comparison
        if ((int)$rachma->designer_id !== (int)$designer->id) {
            abort(403, 'Unauthorized access to this rachma.');
        }
        ...
        try {
            ...
            // Handle preview image deletions first
            if ($request->has('remove_preview_images') && is_array($request->remove_preview_images)) {
                $currentImages = $rachma->preview_images ?? [];
                $imagesToKeep = [];
                
                Log::info('Image deletion request received', [
                    'urls_to_remove' => $request->remove_preview_images,
                    'current_images' => $currentImages
                ]);
                
                // Convert URLs back to storage paths for comparison
                $imagesToRemove = array_map(function($url) {
                    // Extract path from URL: "http://localhost/storage/rachmat_previews/image.jpg" -> "rachmat_previews/image.jpg"
                    if (str_contains($url, '/storage/')) {
                        // Find the position of '/storage/' and extract everything after it
                        $storagePos = strpos($url, '/storage/');
                        if ($storagePos !== false) {
                            $extractedPath = substr($url, $storagePos + 9); // 9 = length of '/storage/'
                            Log::info('URL to path conversion', ['url' => $url, 'path' => $extractedPath]);
                            return $extractedPath;
                        }
                    }
                    Log::info('No storage prefix found in URL', ['url' => $url]);
                    return $url; // fallback to original if no storage prefix found
                }, $request->remove_preview_images);
                
                Log::info('Converted paths to remove', ['paths' => $imagesToRemove]);
                ...
                foreach ($currentImages as $image) {
                    if (in_array($image, $imagesToRemove)) {
                        ...
                        if (Storage::disk('public')->exists($image)) {
                            ...
                            Log::info('Successfully deleted preview image', ['path' => $image]);
                        } else {
                            Log::warning('Preview image file not found for deletion', ['path' => $image]);
                        }
                    } else {
                        // Keep this image
                        $imagesToKeep[] = $image;
                ...
                $updateData['preview_images'] = $imagesToKeep;
                Log::info('Preview images after deletion', ['remaining' => $imagesToKeep]);
            }

            // Handle file deletions
            if ($request->has('remove_files') && is_array($request->remove_files)) {
                $currentFiles = $rachma->files ?? [];
                $filesToKeep = [];
                
                foreach ($currentFiles as $file) {
                    if (in_array($file['id'], $request->remove_files)) {
                        // Delete the file
                        if (isset($file['path'])) {
                ...
                $updateData['files'] = $filesToKeep;
            }

            // Add new preview images if provided
            if ($request->hasFile('preview_images')) {
                $currentImages = $updateData['preview_images'] ?? $rachma->preview_images ?? [];
                
                // Store new images and add to existing ones
                foreach ($request->file('preview_images') as $image) {
                    $currentImages[] = $image->store('rachmat/preview_images', 'public');
                }
                $updateData['preview_images'] = $currentImages;
            }

            // Add new files if provided
            if ($request->hasFile('files')) {
                $currentFiles = $updateData['files'] ?? $rachma->files ?? [];
                
                // Determine the next file ID
                $nextId = 1;
                if (!empty($currentFiles)) {
                    $maxId = max(array_column($currentFiles, 'id'));
                    $nextId = $maxId + 1;
                }

                // Store new files and add to existing ones
                foreach ($request->file('files') as $index => $file) {
                    $filePath = $file->store('rachmat_files', 'private');
                    $extension = strtoupper($file->getClientOriginalExtension());
                    ...
                    $currentFiles[] = [
                        ...
                        'format' => $extension,
                        'size' => $file->getSize(),
                        'is_primary' => empty($currentFiles) && $index === 0, // First file is primary only if no existing files
                        'uploaded_at' => now()->toISOString(),
                        'description' => "ملف بصيغة {$extension}"
                    ];
                }
                $updateData['files'] = $currentFiles;
            }

            $rachma->update($updateData);
            ...
            return redirect()->route('designer.rachmat.index')
                ->with('success', 'تم تحديث الرشمة بنجاح! تم تطبيق جميع التغييرات');

        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Failed to update rachma', [
                'error' => $e->getMessage(),
                'rachma_id' => $rachma->id
            ]);
            ...
            return redirect()->back()
                ->withInput()
                ->with('error', 'حدث خطأ أثناء تحديث الرشمة. يرجى المحاولة مرة أخرى.');
        }
    }
