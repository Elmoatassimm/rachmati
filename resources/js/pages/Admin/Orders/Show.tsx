        ...
        <div className="p-8 space-y-10">
          ...
          <ErrorBoundary>
            <Card className="group relative overflow-hidden border-0 bg-gradient-to-br from-card via-card to-muted/20 shadow-2xl">
              ...
              <CardContent className="relative">
              {(() => {
                // Get rachma data - prioritize order_items for new orders, fallback to legacy rachma field
                const rachmaData = order.order_items && order.order_items.length > 0
                  ? order.order_items[0].rachma
                  : order.rachma;

                // Get price data - use order_items price for new orders, fallback to rachma price
                const priceData = order.order_items && order.order_items.length > 0
                  ? order.order_items[0].price
                  : order.rachma?.price || 0;

                if (!rachmaData) {
                  return (
                    <div className="p-6 text-center text-muted-foreground">
                      <Package className="w-12 h-12 mx-auto mb-4 opacity-50" />
                      <p>لا توجد معلومات رشمة متاحة لهذا الطلب</p>
                    </div>
                  );
                }
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-lg text-foreground">
                          {rachmaData.title_ar || rachmaData.title_fr || rachmaData.title || 'غير محدد'}
                        </p>
                      </div>

                      {(rachmaData.description_ar || rachmaData.description_fr || rachmaData.description) && (
                        <div>
                      ...
                      <div>
                        ...
                        <p className="text-foreground">
                          {rachmaData.categories?.[0]?.name_ar || rachmaData.categories?.[0]?.name_fr || rachmaData.categories?.[0]?.name || 'غير محدد'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-lg text-foreground">{rachmaData.designer?.store_name || 'غير محدد'}</p>
                        <p className="text-sm text-muted-foreground">{rachmaData.designer?.user?.name || 'غير محدد'}</p>
                        <p className="text-sm text-muted-foreground">{rachmaData.designer?.user?.email || 'غير محدد'}</p>
                      </div>
                      ...
                      <ErrorBoundary>
                        {(() => {
                          try {
                            ...
                            const images = rachmaData.preview_image_urls;
                            ...
                            return (
                              <div>
                                <label className="text-sm font-medium text-muted-foreground">صور المعاينة</label>
                                <p className="text-sm text-muted-foreground mt-2">لا توجد صور معاينة</p>
                              </div>
                            );
                          } catch (error) {
